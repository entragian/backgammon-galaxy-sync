 ...
 * "get me a bg-app-token" dance — saved-session reuse, Edge/Chromium launch,
...
const TOKEN_COOKIE = 'bg-app-token';
...
function readTokenFromSession(sessionFile) {
  ...
  try {
    ...
    const tokenCookie = session.cookies?.find(c => c.name === TOKEN_COOKIE);
...
async function getTokenViaBrowser({ sessionFile, loginUrl, log }) {
  ...
  try {
    ...
    const cookies = await context.cookies();
    let token = cookies.find(c => c.name === TOKEN_COOKIE)?.value;
    if (!token) {
